...
#pragma region Proc
LRESULT __stdcall Window::WindowProcInternal(HWND _hWindow, UINT _msg, WPARAM _wp, LPARAM _lp)
{
	if (_msg == WM_NCCREATE)
	{
...
LRESULT __stdcall Window::WindowProc(HWND _hWindow, UINT _msg, WPARAM _wp, LPARAM _lp)
{
	if (_hWindow == nullptr)
		return false;
	switch (_msg)
...
{
	if (menus.contains(_menu->Name().c_str()))
		return;
