...
void CommandManager::DisplayAllHelp() const
{
	...
	{
		if (_command.first == "" || _command.second == nullptr)
			continue;
...
void CommandManager::Register(ICommand* _command)
{
	...
	if (commands.contains(_commandName))
		throw std::exception(std::format("[Command Manger] => {} already exist !", _commandName).c_str());
...
void CommandManager::ExecuteCommand(const std::string& _label, const std::vector<std::string>& _args)
{
	...
	if (!_command->Execute(_args))
		std::cout << _command->Help() << std::endl;
	else
		std::cout << std::endl << std::endl;
}
void CommandManager::DisplayNotReconized(const std::string& _label)
{
