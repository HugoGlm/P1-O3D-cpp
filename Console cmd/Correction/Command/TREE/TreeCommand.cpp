...
void TreeCommand::RecursiveDirectory(const std::string& _path, int _index)
{
    ...
    for (const fs::directory_entry& _entry : fs::directory_iterator(_path, _options))
    {
        ...
        if (_path.has_extension() || !_entry.exists() || !_entry.is_directory())
            continue;
        if (_index > 0)
        {
...
{
    return "Graphically displays the directry structure of a drive or path.";
}
std::string TreeCommand::AdvancedHelp() const
{
    return Help() + "\n\nTREE [drive:][path]";
}
#pragma endregion